...
const reminder = require('./handlers/reminderHandler');
...
module.exports = function (server, restify) {
    ...
    // reminder
    //get  reminder
    server.get('/reminder/:id', restify.plugins.conditionalHandler([
        { version: '1.0.0', handler: reminder.getOneReminderHandler }
    ]));

    //create  reminder
    server.post('/reminder', restify.plugins.conditionalHandler([
        { version: '1.0.0', handler: reminder.createReminderHandler }
    ]));

    //create  remindercreateandclose
    server.post('/remindercreateandclose', restify.plugins.conditionalHandler([
        { version: '1.0.0', handler: reminder.createAndCloseReminderHandler }
    ]));

    //create  reminderclose
    server.post('/reminderclose', restify.plugins.conditionalHandler([
        { version: '1.0.0', handler: reminder.updateReminderHandler }
    ]));

    //create Treatment Reminder
    server.post('/createtreatmentreminder', restify.plugins.conditionalHandler([
        { version: '1.0.0', handler: reminder.createTreatmentReminder }
    ]));

    // // edit  reminder
    server.put('/reminder/:id', restify.plugins.conditionalHandler([
        { version: '1.0.0', handler: reminder.updateReminderHandler }
    ]));

      //search reminders
    server.get('/reminders', restify.plugins.conditionalHandler([
        { version: '1.0.0', handler: reminder.searchReminderHandler }
    ]));
     //Count reminder
     server.get('/remindercount', restify.plugins.conditionalHandler([
        { version: '1.0.0', handler: reminder.countReminderHandler }
    ]));
