...
user.createUserHandler = async function (req, res, next) {
    ...
    try {
        result = await users.createUser(userObj);
...
/*
    getUsersHandler find all user from  DB 
    @params:    req: request object, 
                res: response object, 
                next: callback method,        
*/

user.getUsersHandler = async function (req, res, next) {
    log.info("userHandler.js, Handlers: getUsersHandler");
    let userObj = {};
    try {
...
user.updateUserHandler = async function (req, res, next) {
    ...
    try {
        result = await users.updateUser(id, userObj);
        // if data exist and record found
        if (typeof result != "undefined") {
            res.send(200, { data: result });
...
user.searchUserHandler = async function (req, res, next) {
    ...
    try {
         result = await users.searchUser(condition, {});
        // if data exist and record found
        if (typeof result != "undefined") {
            res.send(200, { data: result });
