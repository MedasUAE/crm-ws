...
/*
    createReminderHandler create new reminder in the DB 
...
reminderHandler.createReminderHandler = async function (req, res, next) {
    ...
    try {
        reminder = await reminderModel.findOneReminder(condition, {});
        reminderData = setReminderData(req.body);
        reminderData["date"] = reminderDate;
        reminderData["message"] = setRemindarMessage(req.body.type);
        if (reminder) {
            result = await reminderModel.updateReminder(reminder._id, reminderData);
            remarkData = setRemarkData(req.body);
            remarkData["reminderDate"] = reminderDate;
            ...
            reminderResult = await reminderModel.createReminder(reminderData);
            remarkData = setRemarkData(req.body);
            remarkData["reminderDate"] = reminderDate;
            if (req.body.remark) remarkResult = await remarkModel.createRemark(remarkData);
            if (reminderResult) results.reminderResult = reminderResult;
...
/*
    createTreatmentReminder create new Treatment reminder in the DB 
    @params:    req: request object, 
                res: response object, 
                next: callback method,        
*/
...
reminderHandler.createTreatmentReminder = async function (req, res, next) {
    ...
    try {
        ...
        if (isServiceId) {
            reminder = await reminderModel.findOneReminder(condition, {});
            if(reminder){
                result = await reminderModel.updateReminder(reminder._id, reminder);
   ...
   remarkData["reminderDate"] =  reminderData["date"];
   washing = await reminderModel.createReminder(reminderData);
   ...
   laser1 = await reminderModel.createReminder(reminderData);
   ...
   medicine = await reminderModel.createReminder(reminderData);
   ...
   remarkData["remark"] = "Second laser remark";
   remarkData["reminderDate"] =  reminderData["date"];
   laser2 = await reminderModel.createReminder(reminderData);
  ...
  //  remarkData["remark"] = "3rd laser remark";
    ...
    laser3 = await reminderModel.createReminder(reminderData);
...
reminderHandler.createAndCloseReminderHandler = async function (req, res, next) {
    ...
    let results = {};
    try {
        updateResult = await reminderModel.updateReminder(req.body.reminderId, { status: 'Close' });
        reminderData = setReminderData(req.body);
...
/*
    getOneReminderHandler find one reminder from  DB 
    @params:    req: request object, 
                res: response object, 
                next: callback method,        
*/
...
reminderHandler.getOneReminderHandler = async function (req, res, next) {
    ...
    try {
        //get reminder from reminder collection
        result = await reminderModel.findOneReminder({ customerId: req.params.id }, {});
        // if data exist and record found
...
/*
    updateReminderHandler update an existing reminder in the DB 
    @params:    req: request object, 
                res: response object, 
                next: callback method,        
*/
...
reminderHandler.updateReminderHandler = async function (req, res, next) {
    ...
    try {
        reminderResult = await reminderModel.updateReminder(id, { status: 'Close' });
        remarkData = setRemarkData(req.body);
       // remarkData["reminderDate"] =  reminderData["reminderDate"];
       remarkData["reminderDate"] =  req.body.date;
...
/*
    searchReminderHandler search reminder in the DB 
    @params:    req: request object, 
                res: response object, 
                next: callback method,        
*/
...
reminderHandler.searchReminderHandler = async function (req, res, next) {
  ...
  if (req.query.createdById) condition["createdFor.id"] = parseInt(req.query.createdById);
   ...
   // else condition.date = moment().format("DD-MM-YYYY");
    ...
    try {
        result = await reminderModel.searchReminder(condition, {});
        // if data exist and record found
        if (typeof result != "undefined") {
            res.send(200, { data: result });
...
/*
    countReminderHandler count reminder in the DB 
    @params:    req: request object, 
                res: response object, 
                next: callback method,        
*/
...
reminderHandler.countReminderHandler = async function (req, res, next) {
    ...
    try {
        result = await reminderModel.countReminder(condition);
        // if data exist and record found
        if (typeof result != "undefined") {
...
function setReminderData(data) {
    ...
    if(data.enteredById) reminderData["createdFor.id"] = data.enteredById;
    if(data.enteredByName) reminderData["createdFor.name"] = data.enteredByName;
...
function setRemarkData(data) {
   ...
   // if (data.reminderDate) remarkData.reminderDate = data.reminderDate;
