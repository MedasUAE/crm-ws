...
remarkHandler.searchRemarkHandler = async function (req, res, next) {
    ...
    if (!req.query.opNo) {
        res.send(400, { error: "No Op Number Found" }); //ToDo Error Code
        return next();
    };
     if (req.query.opNo) condition["patient.opNo"] = req.query.opNo;
    if (req.query.name) condition["patient.name"] = req.query.name;
    if (req.query.consultId) condition["patient.consultId"] = req.query.consultId;
    if (req.query.consultId) condition["createdBy.id"] = req.query.createdById;
